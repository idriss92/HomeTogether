...
function GetDatas_FN(ONSUCCESS_FN,ONERROR_FN){
	var req = {
		'url': zipabox.baseURL+this.uri_set,
                'jar': jar,
                'Content-type' : 'application/json',
	...
	GetDatas(req,function( err, res, body){
...
function GetDeviceValue_FN(uuid,attribute_id,ONSUCCESS_FN,ONERROR_FN){
	var device = GetDeviceByUUID_FN(uuid);
				...
				attr.getdatas(
					function(err, res, body){
						try {
							...
							if(ONERROR_FN)
								ONERROR_FN("[GetDeviceValue] ERROR : " + catchederr + "\r\n" + body);
							return;
						}
						if (ONSUCCESS_FN)
							ONSUCCESS_FN(result.value);
					},
					function(err, res, body){
						if(ONERROR_FN)
							ONERROR_FN(err);
		...
		case "lights" :
			for (var uuid in device.json){
				...
				for (var attr in devicejson.attributes) {				;
					...
					devicejson.attributes[attr].getdatas = GetDatas_FN;
...
var zipabox = {
	...
	GetDeviceLogs: GetDeviceLogs_FN,

	SetDeviceValue: SetDeviceValue_FN,
	GetDeviceValue: GetDeviceValue_FN,
